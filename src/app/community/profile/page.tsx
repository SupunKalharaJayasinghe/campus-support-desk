...
export default function CommunityProfilePage() {
    ...
    const confirmCurrentPostDelete = useCallback(async () => {
        ...
        try {
            setUserPostsError(null);
            const res = await fetch(
                `/api/community-posts/${encodeURIComponent(pending.id)}?userId=${encodeURIComponent(userId)}`,
                { method: "DELETE" }
            );
            if (!res.ok) {
                const body = (await res.json().catch(() => null)) as { error?: string } | null;
            ...
            setUserPosts((prev) => (prev ? prev.filter((p) => p._id !== pending.id) : prev));
    ...
    }, [postDeleteConfirm, profileData.userId, loadDbCommunityProfile]);
    ...
    const toggleOpenPostReplies = useCallback((postId: string) => {
        setExpandedOpenPostReplies((prev) => ({
            ...prev,
            [postId]: !prev[postId],
        }));
    }, []);
    ...
    useEffect(() => {
        if (!draftInUpdateModal && !draftDeleteConfirm && !draftPostConfirm && !postDeleteConfirm)
            return;
                ...
                <div className="flex h-full pt-16">
                                        ...
                                        .map((post) => (
                                            ...
                                            >
                                                ...
                                                <div className="mt-3 flex items-center gap-4 text-xs font-semibold text-slate-600">
                                                    ...
                                                    <button
                                                        type="button"
                                                        onClick={() => toggleOpenPostReplies(post._id)}
                                                        aria-expanded={Boolean(expandedOpenPostReplies[post._id])}
                                                    ...
                                                    >
                                                        ...
                                                        <ChevronDown
                                                            ...
                                                            className={`transition-transform ${
                                                                expandedOpenPostReplies[post._id]
                                                                    ? "rotate-180"
                                                                    : ""
                                                            }`}
                                                        />
                    ...
                    <div id="resolved-posts" className="mt-10 scroll-mt-6">
                        ...
                        <div className="space-y-4">
                            ...
                            <Link
                                href="/community/profile/posts/resolved"
                                className="block w-full rounded-2xl border border-dashed border-green-300 bg-green-50 py-3 text-center text-sm font-semibold text-green-900 transition hover:bg-green-100"
                                onClick={closeSidebarIfMobile}
                            >
                                <span className="inline-flex items-center justify-center gap-2">
                                    <Eye size={15} />
                                    View all resolved posts
                                </span>
                            </Link>
                        </div>
                    </div>
                ...
                >
                    ...
                    >
                        ...
                        <p className="mt-3 text-sm text-slate-600">
                            Are you sure you want to delete “{postDeleteConfirm.title}”? Replies and
                            likes for this post will be removed. This cannot be undone.
                        </p>
                        <div className="mt-6 flex flex-wrap justify-end gap-2">
                            ...
                            <button
                                ...
                                onClick={() => {
                                    confirmCurrentPostDelete().catch(() => undefined);
                                }}
                                disabled={!!deletingPostId}
                            >
