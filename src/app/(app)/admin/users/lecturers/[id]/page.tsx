...
function parseLecturer(payload: unknown): LecturerRecord | null {
...
export default function LecturerProfilePage() {
  ...
  const loadProfile = useCallback(async () => {
    ...
    try {
      const payload = await readJson<unknown>(
        await fetch(`/api/lecturers/${encodeURIComponent(lecturerId)}`, {
          cache: "no-store",
        })
      );
      const parsed = parseLecturer(payload);
      if (!parsed) {
        throw new Error("Failed to parse lecturer profile");
      }
  ...
  const loadOfferings = useCallback(async () => {
    ...
    try {
      const payload = await readJson<unknown>(
        await fetch(`/api/lecturers/${encodeURIComponent(lecturerId)}/offerings`, {
          cache: "no-store",
        })
      );
  ...
  const loadEligibilityLookups = useCallback(async (record: LecturerRecord) => {
    try {
  ...
  useEffect(() => {
    ...
    void loadEligibilityLookups(lecturer);
  }, [lecturer, loadEligibilityLookups]);
              ...
              <div>
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {lecturer.degreeProgramIds.length === 0 ? (
                    ...
                    lecturer.degreeProgramIds.map((code) => (
                      <span
                        className="rounded-full border border-border bg-tint px-3 py-1 text-xs text-heading"
                        key={code}
                      >
