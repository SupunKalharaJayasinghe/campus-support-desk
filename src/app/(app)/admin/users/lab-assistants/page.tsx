...
export default function LabAssistantsPage() {
  ...
  return (
    <div className="space-y-6 lg:space-y-8">
      <PageHeader actions={<Button className="h-11 min-w-[190px] justify-center gap-2 rounded-2xl bg-[#034aa6] px-5 text-white shadow-[0_8px_24px_rgba(3,74,166,0.24)] hover:bg-[#0339a6]" onClick={openAdd}><Plus size={16} />Add Lab Assistant</Button>} description="Manage lab assistant profiles and practical support eligibility scope" title="Lab Assistants" />
      <Card className={cn("transition-all", overlayOpen ? "pointer-events-none opacity-45 blur-[1px]" : "")}>
        <div className="flex flex-col gap-4 border-b border-border pb-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="grid flex-1 gap-3 lg:grid-cols-[minmax(0,1.6fr)_220px_220px]">
            ...
            <div className="flex flex-col gap-1.5"><label className="text-xs font-semibold uppercase tracking-[0.08em] text-text/60">Status</label><Select className="h-12" onChange={(e) => { setStatus(e.target.value as "" | LabAssistantStatus); setPage(1); }} value={status}><option value="">All</option><option value="ACTIVE">ACTIVE</option><option value="INACTIVE">INACTIVE</option></Select></div>
          ...
          <div className="rounded-2xl border border-border bg-tint px-4 py-3"><p className="text-xs font-semibold uppercase tracking-[0.08em] text-text/55">Total Assistants</p><p className="mt-1 text-2xl font-semibold text-heading">{total}</p></div>
        </div>
